  ...
  render: (data = "") ->
    return new Promise (resolve, reject) =>
      ...
      context = {
        ...
        deferRedirect: @url + '/client/defer.js'

        resolve: -> deferred = false; nightmare.run(renderer); resolve(path)
        resolveUrl: @url + '/resolve'
        resolveRedirect: "data:text/plain,"

        reject: -> deferred = false; reject()
        rejectUrl: @url + '/reject'
        rejectRedirect: "data:text/plain,"
      }

      onResourceRequested = (requestData, networkRequest, context) ->
        if requestData.url is context.dataUrl
          networkRequest.changeUrl(context.dataRedirect)

        if requestData.url is context.deferUrl
          context.defer()
          networkRequest.changeUrl(context.deferRedirect)

        if requestData.url is context.resolveUrl
          context.resolve()
          networkRequest.changeUrl(context.resolveRedirect)

        if requestData.url is context.rejectUrl
          context.reject()
          networkRequest.changeUrl(context.rejectRedirect)

      nightmare
        .on('resourceRequestStarted', onResourceRequested, (->), context, (->))
        .viewport(1000, 1000)
        .zoom(2)
        .goto(@url)
        .wait()
